...
class PatelinnisMenu
{
	...
	function enqueue_menu_scripts() {
		...
		if( $cs->id == 'posts_page_order' ){
			...
			wp_enqueue_script( 'order-cats', get_stylesheet_directory_uri() . '/js/src-admin/order-cats.js', array('jquery'), '1.0', true );
    ...
    function all_menu_items_on_blog( $query ){
    	...
    	// make sure all posts/menuitems show on menu
  		...
  		if( ( $query->is_main_query() && isset( $query->query['pagename'] ) && ( $query->query['pagename'] == 'menu' ) ) || ( is_tag() ) ){
	...
	function filter_post_data( $data , $postarr ) {
		...
		if( isset( $_POST ) && !empty( $_POST ) ){
			...
			if( ( isset( $_POST['post_type'] ) ) && ( $_POST['post_type'] == 'post' ) ){
				...
				//this will show you incoming post data
				//look( $_POST );
				//die();
			}		
		}
	...
	function save_post_meta( $post_id, $post, $update ) {
		...
		if( ! ( wp_is_post_revision( $post_id) || wp_is_post_autosave( $post_id ) ) ) {
			...
			if ( 'trash' != get_post_status( $post_id ) && ( !empty( $_POST ) )  ) {
				...
				if( isset( $_POST['post_type'] ) && ( $_POST['post_type'] == 'post' ) ){
					...
					//toggle GF 
					$this->toggleGlutenFreeTax( $post_id );
		...
		} // end checking for revision or auto save
	} 


	//Check for if category is fresh homemade pizza. If it is, then check if the gluten free option is selected. If gf is selected, then add it as a taxonomy. We will then use a taxonomy fee to display the glutten free menu
	private function toggleGlutenFreeTax( $post_id ){

		// //check for gluten free
		if( isset( $_POST['catselect'] ) && ( $_POST['catselect'] == "Fresh Homemade Pizza" ) ){

			$gf_input = $_POST['acf']['field_58f77e4422903'];

			if( $gf_input == "1" ){
				//user has toggle gf input

				//test if gluten free term exisits already in post tags
				$gf_tag_id = term_exists( 'gluten free', 'post_tag' );

				if( !is_null( $gf_tag_id ) ){
					//already exists, check if applied to post
					if( !has_term( 'gluten free', 'post_tag' ) ){
						wp_set_post_terms( $post_id, 'gluten free', 'post_tag', TRUE );
					}
				} else {
					//doesnt exist, create term and add to post
					wp_set_post_terms( $post_id, 'gluten free', 'post_tag', TRUE );
				}
			} else {
				// gf input is not toggled, check if term is applied
				if( has_term( 'gluten free', 'post_tag' ) ){
					//term was applied to post, now remove it.
					wp_remove_object_terms( $post_id, 'gluten free', 'post_tag' );
				}
			}
